...
- 설명 카드 `ActtubCTA` — 왜 acttub 으로 가는지 한 문단
- 화면 하단 고정 버튼 — 본인에게는 "acttub 시작하기", 친구 결과를 보러 온
  방문자에게는 "나도 풀어보기" (먼저 자기 유형을 뽑게 하고 그 다음에 넘긴다)

링크와 이동 함수는 `src/lib/acttub.ts` 한 곳(`ACTTUB_URL`, `openActtub`)에 모아 뒀다.
GA 이벤트는 `acttub_cta`.
